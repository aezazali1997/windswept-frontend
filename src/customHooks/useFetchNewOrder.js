...
const UseFetchNewOrder = ({ selectedOrder, readOnly }) => {
    ...
    useEffect(() => {
        if (selectedOrder) {
            ...
            items.map(({ product, material, backing, pe, setQty }) => {
    ...
    const ConfirmSubmit = (data) => {
        Swal.fire({
            ...
            customClass: {
                ...
                denyButton: 'w-full inline-flex justify-center rounded-md border-none px-4 py-2 btn  text-base font-medium text-white focus:outline-none sm:ml-3 sm:w-auto sm:text-sm'
            }
        }).then((result) => {
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    buttonsStyling: false,
                    customClass: {
                        confirmButton: 'w-full inline-flex justify-center rounded-md border-none btn px-4 py-2  text-base font-medium text-white focus:outline-none sm:ml-3 sm:w-auto sm:text-sm',
                    }
                }).then((result) => {
                    if (result.isConfirmed) {
    ...
    return {
        _GrandTotalWithMarkup, _grandTotal, _Total, _onBlur, _onFocus, showFormDetails, onCancleOrder, removeItem,
        addAnother, handlePMSModal, handleThreadModal, removeColor, handleColors, handleQty, filterOptions, _HandleChange,
        handleNotes, handleRemoveImg, handleRemoveOrderImg, OrderUploadClick, onChangeOrderFile, handleClick, onChangeFile,
        setColor, DisableAddAnother, EnableAddAnother, formik, date, week, total, grandTotal, gTotalWithMarkup, loading,
        images, orderImages, apiError, showPMSModal, showThreadModal, data, errors, values, notes, color, selected, orderNo,
        orderUpload, upload, canAddAnother
