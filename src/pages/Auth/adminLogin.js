...
const AdminLogin = () => {
    ...
    const _OnSubmit = async (email, password, setStatus, setSubmitting) => {
        enableLoading();
        try {
            const { data: { data: { token }, message } } = await AxiosInstance.adminLogin({
                email,
                password
            })
            disableLoading();
                ...
                <form className="mt-8" onSubmit={formik.handleSubmit} >
                    ...
                    <div className="flex items-center justify-end">
                        ...
                        <button
                            ...
                            className={`flex items-center text-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white ${formik.isSubmitting ? 'bg-red-500 cursor-not-allowed pointer-events-none' : ' bg-red-600 hover:bg-red-700 '}focus:outline-none`}
                        >
                            Log in
                            {loading &&
