...
const NewOrder = ({ readOnly, selectedOrder, closeOrder }) => {
	...
	const { _onBlur, _onFocus, showFormDetails, onCancleOrder, removeItem,
		addAnother, handlePMSModal, handleThreadModal, removeColor, handleColors, handleQty, filterOptions, _HandleChange,
		handleRemoveImg, handleRemoveOrderImg, OrderUploadClick, onChangeOrderFile, handleClick, onChangeFile, setColor,
		formik, date, week, total, grandTotal, gTotalWithMarkup, loading, images, orderImages, apiError, showPMSModal,
		showThreadModal, data, errors, values, color, selected, orderNo, orderUpload, upload, EnableAddAnother,
		DisableAddAnother, canAddAnother
				...
				>
					<div className="flex flex-col w-full h-full justify-center items-center md:sticky md:top-2">
						<div className="w-full h-full px-3 sm:w-2/3">
													...
													values && !isEmpty(values) ?
														values.map((item, index) => (
															<div key={index}
																...
																<div className="flex items-center w-full" onClick={() => showFormDetails(index)}>
							...
							<div className="flex flex-col w-full ">
								<button
									...
									onClick={addAnother} type="button"
									className={`inline-flex 
									${readOnly ? 'bg-red-500 cursor-default' : canAddAnother ? 'bg-red-500 cursor-default' : 'bg-red-600 hover:bg-red-700'}
						...
						<div className="flex flex-col w-full px-3 py-4 sm:w-2/3 border rounded-md space-y-3">
							<div className={`${isEmpty(images) !== true ? 'grid grid-cols-3 sm:grid-cols-4 gap-2' : 'flex justify-center'} w-full`}>
								{
									!isEmpty(images) ?
